 ...
 * Martin Pihrt - 22.08.2022
 * Arduino IDE >= 1.8.19, Select board: doit ESP32 devkit V1
 ...
 *  BUTTON AP MANAGER -> It must be held for at least >4 seconds when switching on (for starting AP manager) on time 10 minutes
 *  Manual switch relay 1 to on/off with AP button (if press time <4 seconds) on HW1.0 board
 *  
 *  Manual switch relay 1 or 2 to on/off with button RE1 and RE2 on HW1.1 board
 *  
...
/* Serial line (request from a serial line)
   "AP|" for starting AP manager
   "RBT|" for rebooting
*/
...
// FW version
int fw_version = 119;        // ex: FW version 119 is 1.19
/* 
 *  
 * FW 1.19: Added new type "multiswitch for 7 buttons" as controller for running programs in OSPy
 * FW 1.18: Added support for hardware board HW:1.1 (multisensor by pihrt.com). 
            New feture: two buttons for manual control for two relay. 2x LED for relays ON state. 
            1x LED for 3,3V power supply status. Contolling two relay via web server (from OSPy) instead one relay.
            Fixed: random clickering in relay (resistor in gate for FET 2N7002). Inductor package size on PCB.
            If GPIO4 and GPIO15 have pull-up resistors (state on GPIOs is on VCC 3,3V) then automatic detect and switch HW version to "1" HW1.1
            Added I2C (0x3c) OLED display 0,96" 128x64 (SSD1306) to display measured values from all inputs except humidity (DHTxx) and ultrasound (because these are on the I2C connector and cannot be used at the same time as the display).
            New parameters for web:           
            example for on relay 1 to 400 second:
            http://192.168.88.207/0123456789abcdef?re=1&run=400
            example for on relay 2 to 10 second:
            http://192.168.88.207/0123456789abcdef?re_2=1&run_r2=10
 * FW 1.17: Fixed soil moisture (4x ADS1115) on I2C bus. Add sensor FWxx version to AP manager webpage.
 * FW 1.16: Fixed bug in sensor "motion" and "dry-contact" (send state after power on if input has not changed). 
 * FW 1.15: Added support for 16 pcs soil moisture (4x ADS1115) on I2C bus address: 0x48,0x49,0x4A,0x4B. A/D range 6.144 volt
            Added support serial line (processing a request from a serial line) for example: AP| for starting AP manager, RBT| for rebooting
 * FW 1.14: AP manager - time for invocation: the time for calling up the AP manager with the button was increased to 4 seconds (at a time of 2 seconds, when manually controlling the relay with the button, the AP manager was called up by mistake, even if the user did not want it). 
 * FW 1.13: Fixed value refresh for calibration of temperature sensors DS1-DS4. Remove hw timer for LED blinking
...
byte out_relay_2       = 5;                      // relay 2 for control on/off from OSPy
bool revers            = false;                  // if true is active out in LOW, false is active out in HIGH (for inversion relay logic)

// Button for manual relays control
byte TL_1              = 4;                      // button 1 for manual control on/off relay 1 
byte TL_2              = 15;                     // button 2 for manual control on/off relay 2
...
// OLED
// https://dratek.cz/arduino/1569-iic-i2c-oled-display-0-96-128x64-bily.html
#include "SSD1306.h"                             // alias for #include "SSD1306Wire.h"
SSD1306  display(0x3c, I2C_SDA, I2C_SCL);
/* menu
show_on_OLED(0);   // ospy 
show_on_OLED(1);   // AP manager
show_on_OLED(2);   // wifi connection
show_on_OLED(3);   // FW version
show_on_OLED(4);   // HW version
show_on_OLED(5);   // DS1
show_on_OLED(6);   // DS2
show_on_OLED(7);   // DS3
show_on_OLED(8);   // DS4
show_on_OLED(9);   // dry contact
show_on_OLED(10); // leak
show_on_OLED(11); // motion
show_on_OLED(12); // IP
show_on_OLED(13); // MAC
show_on_OLED(14); // RSSI
show_on_OLED(15); // VCC
show_on_OLED(16); // reboot
*/
...
typedef struct{
  ...
  char SEN_TYPE[4];       // 5 is temperature sensor (1=Dry Contact, 2=Leak Detector, 3=Moisture, 4=Motion, 5=temperature, 6=multisensor, 7=multiswitch)
...
int LEDinterval = 1000;                          // default LED blinking interval 1000ms
...
bool relay_status;                               // relay status 1 (0=off, 1=on) for webpages
bool relay_status_2;                             // relay status 2 (0=off, 1=on) for webpages
...
bool apm = false;                                // helper for AP button (if not pushed)  
...
bool change;                                     // manual relay 1 on/off with AP button (if press time < 2000 sec) or tl1 in HW1.1 version
bool change_2;                                   // manual relay 1 on/off with tl2 button
bool TL1pom, TL2pom;                             // helpers for button 1 a 2
bool en_led_blink;                               // enable blinking LED (if relay is OFF
bool OLED_is_connected;                          // true if is connected on I2C bus  
...
unsigned long set_to_on;                         // millis for run time parameter relay 1
unsigned long run_set_to;                        // millis for run time from web server form &run=xxx
unsigned long set_to_on_2;                       // millis for run time parameter relay 2
unsigned long run_set_to_2;                      // millis for run time from web server form &run_r2=xxx
unsigned long preLEDtime;                        // millis for blinking LED
unsigned long OLEDpretime;                       // millis for shift OLED MSG
byte OLEDshift;                                  // OLED shift menu
bool last_sw7_1 = HIGH;                          // last value for 1 switch in mode 7 (SEN_TYPE = 7)
bool last_sw7_2 = HIGH;                          // ...
bool last_sw7_3 = HIGH;                          // ...
bool last_sw7_4 = HIGH;                          // ...
bool last_sw7_5 = HIGH;                          // ...
bool last_sw7_6 = HIGH;                          // ...
bool last_sw7_7 = HIGH;                          // last value for 7 switch in mode 7 (SEN_TYPE = 7)

byte hw_board_version = 0;                       // default is "0" for HW board version 1.0 
// if GPIO4 and GPIO15 have pull-up resistors (state on GPIOs is on VCC 3,3V) then automatic detect and switch HW version to "1" HW1.1

// ADS convertors
int16_t adc0, adc1, adc2, adc3, adc4, adc5, adc6, adc7, adc8, adc9, adc10, adc11, adc12, adc13, adc14, adc15; // number from A/D
float adv0, adv1, adv2, adv3, adv4, adv5, adv6, adv7, adv8, adv9, adv10, adv11, adv12, adv13, adv14, adv15;   // voltage from A/D
bool ads1_err, ads2_err, ads3_err, ads4_err;

// Incomming commands form serial line
String SerialBuffer;
...
unsigned long OLEDinterval = 3000;               // 3 sec shift for OLED
...
void setEEPROMdefault(){ 
   ...
   setEEPROMString(offsetof(eepromdata_t, SEL_MOIST_SONIC), elementSize(eepromdata_t, SEL_MOIST_SONIC), "0");                  // 0=moisture, 1=sonic, 2=soil probe
...
void processCommand(){                      // processing a request from a serial line
  if(SerialBuffer.startsWith("AP")){        // enabling AP manager
    ap_manager = true;                      
    WIFIAP_setup();                          
  }//end if
  if(SerialBuffer.startsWith("RBT")){       // reboot HW
    reboot();
  }//end if  
}//end if  

bool readSerials(){    
    while(Serial.available()){                // get data from serial line
       ...
       if(received == '|'){  // first fill the buffer with a data and then process it with the help of "|"  (example: AP| for starting AP manager)
          #ifdef DEBUG 
             Serial.println(F(""));
             Serial.flush();
          #endif   
...
String str_fw_version(){
  byte ones = (fw_version%10); 
  byte tens = ((fw_version/10)%10);
  byte hundreds = ((fw_version/100)%10);
  return String(hundreds) + "." + String(tens) + String(ones);
}

void show_on_OLED(byte msg_nr){
  if(OLED_is_connected){
    ...
    switch(msg_nr){
      case 0: // home
         display.drawString(64, 10, "OSPy ESP32");   // 0,0 (x,y) on left top window
         display.drawString(64, 25, "MULTISENSOR");  // maximum is 128, 64
         display.drawString(64, 40, "by PIHRT.COM");
      break;
      case 1: // AP manager
         display.drawString(64, 10, "AP manager");   
         display.drawString(64, 25, "exit on " + String(numberOfMinutes(APtimeLCD/1000)) + ":" + String(numberOfSeconds(APtimeLCD/1000)));  
         display.drawString(64, 40, "IP: 192.168.1.1");
      break;
      case 2: // connecting Wi-Fi
         display.drawString(64, 10, "Wi-Fi");   
         display.drawString(64, 25, "connecting");  
         display.drawString(64, 40, "please wait");
      break;
      case 3: // FW version
         display.drawString(64, 10, "Firmware");   
         display.drawString(64, 25, "version");  
         display.drawString(64, 40, str_fw_version());
      break;
      case 4: // HW version
         display.drawString(64, 10, "Hardware");   
         display.drawString(64, 25, "version");
         if(hw_board_version ==0) display.drawString(64, 40, "1.0");
         else display.drawString(64, 40, ">= 1.1");
      break;              
      case 5: // DS1
         display.drawString(64, 10, "Temperature");   
         display.drawString(64, 25, "DS 1");  
         if(curTemp_1 > -100) display.drawString(64, 40, String(curTemp_1)+ "C");
         else display.drawString(64, 40, "No connected");
      break; 
      case 6: // DS2
         display.drawString(64, 10, "Temperature");   
         display.drawString(64, 25, "DS 2");  
         if(curTemp_2 > -100) display.drawString(64, 40, String(curTemp_2)+ "C");
         else display.drawString(64, 40, "No connected");
      break;      
      case 7: // DS3
         display.drawString(64, 10, "Temperature");   
         display.drawString(64, 25, "DS 3");  
         if(curTemp_3 > -100) display.drawString(64, 40, String(curTemp_3)+ "C");
         else display.drawString(64, 40, "No connected");
      break;  
      case 8: // DS4
         display.drawString(64, 10, "Temperature");   
         display.drawString(64, 25, "DS 4");  
         if(curTemp_4 > -100) display.drawString(64, 40, String(curTemp_4)+ "C");
         else display.drawString(64, 40, "No connected");
      break;
      case 9: // Dry contact
         get_dry_contact();
         display.drawString(64, 10, "Dry contact");   
         display.drawString(64, 25, "is now in " + String(curDrcon));  
         if(curDrcon == 1) display.drawString(64, 40, "Closed");
         else display.drawString(64, 40, "Open");
      break;
      case 10: // Leak
         display.drawString(64, 10, "Leak");   
         display.drawString(64, 25, " ");  
         display.drawString(64, 40, String(curLkdet) + "ltr");
      break;
      case 11: // Motion
         get_motion();
         display.drawString(64, 10, "Motion");   
         display.drawString(64, 25, "is now in " + String(curMoti));  
         if(curMoti == 1) display.drawString(64, 40, "Motion");
         else display.drawString(64, 40, "No motion");
      break;
      case 12: // IP
         get_ip();
         display.drawString(64, 10, "IP address");   
         display.drawString(64, 25, " ");  
         display.drawString(64, 40, IPadr);
      break; 
      case 13: // MAC
         display.drawString(64, 10, "MAC address");   
         display.drawString(64, 25, " ");  
         display.drawString(64, 40, String(MACadr));
      break; 
      case 14: // RSSI
         display.drawString(64, 10, "RSSI signal");   
         display.drawString(64, 25, " ");  
         display.drawString(64, 40, String(curPercent) + "%");
      break; 
      case 15: // VCC
         get_adc();
         display.drawString(64, 10, "In power");   
         display.drawString(64, 25, " ");  
         display.drawString(64, 40, String(curVdd) + "V");
      break;
      case 16: // restart
         display.drawString(64, 10, "Restarting");   
         display.drawString(64, 25, "please");  
         display.drawString(64, 40, "wait...");
      break;
      case 17: // FW upgrading
         display.drawString(64, 10, "FW upgrading");   
         display.drawString(64, 25, "please");  
         display.drawString(64, 40, "wait...");
      break;         
      default:
         display.drawString(64, 10, "OSPy ESP32");
         display.drawString(64, 25, "MULTISENSOR");
         display.drawString(64, 40, "by PIHRT.COM");
      break;
    }//end switch
    display.display();
  }//end if OLED_is_connected 
}//end void
...
void setup() {
  ...
  pinMode(TL_2, INPUT);
  en_led_blink = false;
  set_rele(0);                                 // relay 1 off
  set_rele_2(0);                               // relay 2 off

  // where GPIO has pull-up (on VCC 3,3V) is HW version 1.1 
  if(digitalRead(TL_1) && digitalRead(TL_2)) hw_board_version = 1; // HW version >= 1.1 (two relay and buttons)
  else                                       hw_board_version = 0; // HW version = 1.0 (one relay without buttons)
  ...
  for(address = 1; address < 127; address++){
    ...
    if(nDevices == 0) {
      #ifdef DEBUG 
         Serial.println(F("Not found I2C devices")); Serial.flush();
      #endif   
    }   

  // ADS1115 begin
  #ifdef DEBUG
     Serial.print(F("ADS1X15 LIB VERSION: "));
     Serial.println(ADS1X15_LIB_VERSION);
  #endif 
  if(!ADS1.begin()) {
    #ifdef DEBUG
        Serial.println(F("Failed to initialize ADS1115 on 0x48.")); Serial.flush();
    #endif
    ads1_err = true;   // not found ads1
  }
  else{
    ads1_err = false;  // found ads1 OK
    ADS1.setGain(0);   // range 6.144 volt
    #ifdef DEBUG
        Serial.println(F("Initialize ADS1115 on 0x48 OK.")); Serial.flush();
    #endif    
  }
  
  if(!ADS2.begin()) {
    #ifdef DEBUG
        Serial.println(F("Failed to initialize ADS1115 on 0x49.")); Serial.flush();
    #endif
    ads2_err = true;    
  }
  else{
    ...
    ADS2.setGain(0);   
    #ifdef DEBUG
        Serial.println(F("Initialize ADS1115 on 0x49 OK.")); Serial.flush();
    #endif    
  }
    
  if(!ADS3.begin()) {
    #ifdef DEBUG
        Serial.println(F("Failed to initialize ADS1115 on 0x4A.")); Serial.flush();
    #endif
    ads3_err = true;
  }
  else{
    ...
    ADS3.setGain(0);   
    #ifdef DEBUG
        Serial.println(F("Initialize ADS1115 on 0x4A OK.")); Serial.flush();
    #endif    
  }
    
  if(!ADS4.begin()) {
    #ifdef DEBUG
        Serial.println(F("Failed to initialize ADS1115 on 0x4B.")); Serial.flush();
    #endif
    ads4_err = true;
  }
  else{
    ...
    ADS4.setGain(0);   
    #ifdef DEBUG
        Serial.println(F("Initialize ADS1115 on 0x4B OK.")); Serial.flush();
    #endif    
  }
  ...
  error = Wire.endTransmission();
  if(error == 0){
    OLED_is_connected = true;
    display.init();
    #ifdef DEBUG
        Serial.println(F("Initialize OLED on 0x3c OK.")); Serial.flush();
    #endif
    show_on_OLED(0);
    delay(1500);
    // display.flipScreenVertically();
    ...
    // display.drawRect(12, 12, 20, 20);       // Draw rectangle
    // display.fillRect(14, 14, 17, 17);       // Fill the rectangle
    // display.drawHorizontalLine(0, 40, 20);  // Draw a line horizontally
    // display.drawVerticalLine(40, 0, 20);    // Draw a line vertically
    // display.drawCircle(32, 32, i*3);        // Draw circle
    // display.fillCircle(96, 32, 32 - i* 3);  // Draw fill circle
    // int progress = (counter / 5) % 100;
    // display.drawProgressBar(0, 32, 120, 10, progress); // Draw progress bar
    // display.drawString(64, 15, String(progress) + "%");
    // display.drawXbm(34, 14, WiFi_Logo_width, WiFi_Logo_height, WiFi_Logo_bits);
  }
  else{
    OLED_is_connected = false;
    #ifdef DEBUG
        Serial.println(F("Failed to initialize OLED on 0x3c.")); Serial.flush();
    #endif
  }
  ...
  if(!ap_manager){ 
    ...
    while(wifiMulti.run()!=WL_CONNECTED){
       ...
       if(readSerials()){                        // find out if there is any requirement on the serial line           
         return;
       }
    }// end while 
    
    if((wifiMulti.run() == WL_CONNECTED)){       // if Wi-Fi is connected set hostname, show RSSI, MAC and IP
       en_led_blink = true;
       #ifdef DEBUG
  ...
  server.on("/" + String(AES_SEC), [](){                      // ex for on: http://192.168.88.207/0123456789abcdff?re=1&run=400
    if(server.hasArg("re")){    // relay 1    
      ...
      if(server.hasArg("run")){                               // run time parameter in seconds ex for on: http://192.168.88.207/0123456789abcdff?re=1&run=10
         ...
         #ifdef DEBUG
            Serial.print(F("WEB RUNTIME RELAY 1: "));
      ...
      if(o==1) {
         set_rele(1);                                         // relay on
         #ifdef DEBUG
            Serial.println(F("WEB RELAY 1: ON"));
            Serial.flush();
         #endif
         en_led_blink = false;
         digitalWrite(out_led, HIGH);  
         ...
         #ifdef DEBUG
            Serial.println(F("WEB RELAY 1: OFF"));
            Serial.flush();
         #endif
         digitalWrite(out_led, LOW);
         en_led_blink = true; 
    ...
    if(server.hasArg("re_2")){   // relay 2     
      int p = atoi(server.arg("re_2").c_str());
      if(server.hasArg("run_r2")){                            // run time parameter in seconds ex for on: http://192.168.88.207/0123456789abcdff?re_2=1&run_r2=10
         set_to_on_2 = millis();                              // now runtime has theses millis()
         run_set_to_2 = (atoi(server.arg("run_r2").c_str()))*1000; // run time is in seconds -> convert to mseconds
         #ifdef DEBUG
            Serial.print(F("WEB RUNTIME RELAY 2: "));
            Serial.println(run_set_to_2);
            Serial.flush();
         #endif
      }
      else{
      ...
      if(p==1) {
         set_rele_2(1);                                         // relay 2 on
         #ifdef DEBUG
            Serial.println(F("WEB RELAY 2: ON"));
            Serial.flush();
         #endif
         en_led_blink = false;
         digitalWrite(out_led, HIGH);  
         server.send(200, "text/html", String(p));
      }
      else if(p==0) {
         run_set_to_2 = 0;
         set_rele_2(0);                                         // relay 2 off
         #ifdef DEBUG
            Serial.println(F("WEB RELAY 2: OFF"));
            Serial.flush();
         #endif
         digitalWrite(out_led, LOW);
         en_led_blink = true; 
 ...
 -> FALLING Triggers interrupt when the pin goes from HIGH to LOW
  ...
  if(SEL_MOIST_SONIC==2 && SEN_TYPE == 6){                    // reading from soil A/D
      get_adc();
  }//end if

  // reading dry-contact state after power on
  if(INVERT_DRY==0){
    pinMode(input_sensor_dry, INPUT_PULLUP);                  // with pull-up input 
    LastCurDrcon = digitalRead(input_sensor_dry);             // 0 is 0, 1 is 1
  }
  else{
    pinMode(input_sensor_dry, INPUT);                         // only input pull-down
    LastCurDrcon = !digitalRead(input_sensor_dry);            // 0 is 1, 1 is 0
  }
  curDrcon = LastCurDrcon;
  
  // reading motion contact state after power on
  if(INVERT_MOTI==0) {
    pinMode(input_sensor_mot, INPUT_PULLUP);                  // normal pull-up
    LastCurMoti = digitalRead(input_sensor_mot);
  }
...
void loop(){
 readSerials();
 check_manual_btn();                                          // buttons for tl1 (relay1) and tl2 (relay 2) 
 nowTime = millis();                                          // actual millis time

 if(nowTime-preLEDtime >= LEDinterval){
    preLEDtime = nowTime;
    if(en_led_blink){   
       digitalWrite(out_led, !digitalRead(out_led));       
    }// end if
 }//end 100ms and 1000ms timer

 if(nowTime-OLEDpretime >= OLEDinterval){
    OLEDpretime = nowTime;
    if(ap_manager){
       show_on_OLED(1);
    }
       ...
       OLEDshift ++;
       if(OLEDshift > 15) OLEDshift = 3;
       if(OLEDshift < 3)  OLEDshift = 3;
       show_on_OLED(OLEDshift);
    }   
 }//end 3000ms timer
 ...
 if(ap_manager){                                              // manager is active                                   
   ...
   if(run_set_to > 0){                                        // if is inserted parameter &run=xxx from web
     if(millis()-set_to_on >= run_set_to){                    // the difference between the current time and when the power-on request was received
       if(digitalRead(out_relay)==HIGH && !revers){           // if output is in HIGH (not reversing) 
         set_rele(0);                                         // relay 1 off
         #ifdef DEBUG
            Serial.println(F("RUNTIME -> RELAY 1: OFF (GND)"));
            Serial.flush();
         #endif
       }
       if(digitalRead(out_relay)==LOW && revers){             // if output is in LOW (in reversing) 
         ...
         #ifdef DEBUG
            Serial.println(F("RUNTIME -> RELAY 1: OFF (VCC)"));
            Serial.flush();
         #endif         
       }
       en_led_blink = true;
   ...
   if(run_set_to_2 > 0){                                      // if is inserted parameter &run_r2=xxx from web
     if(millis()-set_to_on_2 >= run_set_to_2){                // the difference between the current time and when the power-on request was received
       if(digitalRead(out_relay_2)==HIGH && !revers){         // if output is in HIGH (not reversing) 
         set_rele_2(0);                                       // relay 2 off
         #ifdef DEBUG
            Serial.println(F("RUNTIME -> RELAY 2: OFF (GND)"));
            Serial.flush();
         #endif
       }
       if(digitalRead(out_relay_2)==LOW && revers){           // if output is in LOW (in reversing) 
         set_rele_2(1);                                       // relay 2 off
         #ifdef DEBUG
            Serial.println(F("RUNTIME -> RELAY 1: OFF (VCC)"));
            Serial.flush();
         #endif         
   ...
   if(nowTime - prevTikTime >= 1000){                         // second timer for sensors
    ...
    if(run_set_to > 0){                                       // if is inserted parameter &run=xxx from web
     #ifdef DEBUG
        Serial.print(F("RELAY 1 OFF after: ")); 
    ...
    if(run_set_to_2 > 0){                                     // if is inserted parameter &run_r2=xxx from web
     #ifdef DEBUG
        Serial.print(F("RELAY 2 OFF after: ")); 
        Serial.println((set_to_on_2 + run_set_to_2)-millis());
        Serial.flush();
     #endif 
    }      
    ...
    if(SEL_MOIST_SONIC==2 && SEN_TYPE == 6){                    // print reading from soil A/D
      get_adc();
    }
    
    if(SEN_TYPE == 7){                                          // print 7x button state   
      get_7_buttons();
    }         
  }// end timer 1 sec 
   ...
   if((WiFi.status() == WL_CONNECTED) && send_to_OSPy){         // if Wi-Fi connected and is time for sending
     ...
     String httpRequestData  = "do={\"ip\":\"";
           ...
           if(SEL_MOIST_SONIC==2 && SEN_TYPE == 6){    // soil probes on I2C bus pins
              httpRequestData += "\",\"sm_0\":\"";
              httpRequestData += adv0;
              httpRequestData += "\",\"sm_1\":\"";
              httpRequestData += adv1;
              httpRequestData += "\",\"sm_2\":\"";
              httpRequestData += adv2;
              httpRequestData += "\",\"sm_3\":\"";
              httpRequestData += adv3;
              httpRequestData += "\",\"sm_4\":\"";
              httpRequestData += adv4;
              httpRequestData += "\",\"sm_5\":\"";
              httpRequestData += adv5;
              httpRequestData += "\",\"sm_6\":\"";
              httpRequestData += adv6;
              httpRequestData += "\",\"sm_7\":\"";
              httpRequestData += adv7;
              httpRequestData += "\",\"sm_8\":\"";
              httpRequestData += adv8;
              httpRequestData += "\",\"sm_9\":\"";
              httpRequestData += adv9;
              httpRequestData += "\",\"sm_10\":\"";
              httpRequestData += adv10;
              httpRequestData += "\",\"sm_11\":\"";
              httpRequestData += adv11; 
              httpRequestData += "\",\"sm_12\":\"";
              httpRequestData += adv12;
              httpRequestData += "\",\"sm_13\":\"";
              httpRequestData += adv13;
              httpRequestData += "\",\"sm_14\":\"";
              httpRequestData += adv14;
              httpRequestData += "\",\"sm_15\":\"";
              httpRequestData += adv15;                                                       
           }
           else{
              httpRequestData += "\",\"sm_0\":\"";
              httpRequestData += -127;
           ...
           if(SEN_TYPE == 7){    // 7x multi contact (switch controller)
              httpRequestData += "\",\"sw1\":\"";
              httpRequestData += last_sw7_1;
              httpRequestData += "\",\"sw2\":\"";
              httpRequestData += last_sw7_2;
              httpRequestData += "\",\"sw3\":\"";
              httpRequestData += last_sw7_3;
              httpRequestData += "\",\"sw4\":\"";
              httpRequestData += last_sw7_4;
              httpRequestData += "\",\"sw5\":\"";
              httpRequestData += last_sw7_5;
              httpRequestData += "\",\"sw6\":\"";
              httpRequestData += last_sw7_6;
              httpRequestData += "\",\"sw7\":\"";
              httpRequestData += last_sw7_7;                                                                                    
           }   
           httpRequestData += "\"}";
           httpRequestData += "&sec=";
           String astr="";
...
// HELPERS****************************************************************************************************************
void check_manual_btn(){
   if(hw_board_version >= 1){ // HW1.1 board and more
      if(digitalRead(TL_1)==HIGH) TL1pom = true;
      if(digitalRead(TL_2)==HIGH) TL2pom = true;

      if((digitalRead(TL_1)==LOW)&& TL1pom){ // relay 1
         TL1pom = false;
         change = !change;
         set_rele(change);
         if(change){ 
      ...
      if((digitalRead(TL_2)==LOW)&& TL2pom){ // relay 2
         TL2pom = false;
         change_2 = !change_2;
         set_rele_2(change_2);
         if(change_2){ 
...
void get_7_buttons(){                        // 7x contact buttons as input (active in gnd)
   pinMode(input_sensor_1, INPUT_PULLUP);    // connector DS1 as input
   pinMode(input_sensor_2, INPUT_PULLUP);    // connector DS2
   pinMode(input_sensor_3, INPUT_PULLUP);    // connector DS3
   pinMode(input_sensor_4, INPUT_PULLUP);    // connector DS4
   pinMode(input_sensor_dry, INPUT_PULLUP);  // connector dry
   pinMode(input_sensor_mot, INPUT_PULLUP);  // connector motion
   pinMode(input_sensor_leak, INPUT_PULLUP); // connector leak
   delay(10);
   bool _a = digitalRead(input_sensor_1);    // read input state
   bool _b = digitalRead(input_sensor_2);
   bool _c = digitalRead(input_sensor_3);
   bool _d = digitalRead(input_sensor_4);
   bool _e = digitalRead(input_sensor_dry);
   bool _f = digitalRead(input_sensor_mot);
   bool _g = digitalRead(input_sensor_leak);
   if(_a != last_sw7_1){
      last_sw7_1 = _a;
      #ifdef DEBUG
         Serial.print(F("CHANGE CONTACT SW1 (DS1): "));
         Serial.println(last_sw7_1);
         Serial.flush();
      #endif   
      send_to_OSPy = true;
      interval = 1000;
   ...
   if(_b != last_sw7_2){
      last_sw7_2 = _b;
      #ifdef DEBUG
         Serial.print(F("CHANGE CONTACT SW2 (DS2): "));
         Serial.println(last_sw7_2);
         Serial.flush();
      #endif   
      send_to_OSPy = true;
      interval = 1000;
   ...
   if(_c != last_sw7_3){
      last_sw7_3 = _c;
      #ifdef DEBUG
         Serial.print(F("CHANGE CONTACT SW3 (DS3): "));
         Serial.println(last_sw7_3);
         Serial.flush();
      #endif   
      send_to_OSPy = true;
      interval = 1000;
   ...
   if(_d != last_sw7_4){
      last_sw7_4 = _d;
      #ifdef DEBUG
         Serial.print(F("CHANGE CONTACT SW4 (DS4): "));
         Serial.println(last_sw7_4);
         Serial.flush();
      #endif   
      send_to_OSPy = true;
      interval = 1000;
   ...
   if(_e != last_sw7_5){
      last_sw7_5 = _e;
      #ifdef DEBUG
         Serial.print(F("CHANGE CONTACT SW5 (Dry): "));
         Serial.println(last_sw7_1);
         Serial.flush();
      #endif   
      send_to_OSPy = true;
   ...
   if(_f != last_sw7_6){
      last_sw7_6 = _f;
      #ifdef DEBUG
         Serial.print(F("CHANGE CONTACT SW6 (Mot): "));
         Serial.println(last_sw7_6);
         Serial.flush();
      #endif   
      send_to_OSPy = true;
      interval = 1000;
   ...
   if(_g != last_sw7_7){
      last_sw7_7 = _g;
      #ifdef DEBUG
         Serial.print(F("CHANGE CONTACT SW7 (Leak): "));
         Serial.println(last_sw7_7);
         Serial.flush();
      #endif   
      send_to_OSPy = true;
      interval = 1000;
...
void get_adc(){
   if(!ads1_err){                            // if adc1115 0x48 is conected
      adc0 = ADS1.readADC(0);                // read A/D to number
      adc1 = ADS1.readADC(1);
      adc2 = ADS1.readADC(2);
      adc3 = ADS1.readADC(3);
      adv0 = ADS1.toVoltage(adc0);           // convert number to voltage
      adv1 = ADS1.toVoltage(adc1);
      adv2 = ADS1.toVoltage(adc2);
      adv3 = ADS1.toVoltage(adc3);
   }
   else{
      adv0 = -127; adv1 = -127; adv2 = -127; adv3 = -127;  
   }//end else

   if(!ads2_err){                            // if adc1115 0x49 is conected
      adc4 = ADS2.readADC(0);                // read A/D to number
      adc5 = ADS2.readADC(1);
      adc6 = ADS2.readADC(2);
      adc7 = ADS2.readADC(3);
      adv4 = ADS2.toVoltage(adc4);           // convert number to voltage
      adv5 = ADS2.toVoltage(adc5);
      adv6 = ADS2.toVoltage(adc6);
      adv7 = ADS2.toVoltage(adc7);
   }
   else{
      adv4 = -127; adv5 = -127; adv6 = -127; adv7 = -127;  
   }//end else

   if(!ads3_err){                            // if adc1115 0x4A is conected
      adc8  = ADS3.readADC(0);               // read A/D to number
      adc9  = ADS3.readADC(1);
      adc10 = ADS3.readADC(2);
      adc11 = ADS3.readADC(3);
      adv8  = ADS3.toVoltage(adc8);          // convert number to voltage
      adv9  = ADS3.toVoltage(adc9);
      adv10 = ADS3.toVoltage(adc10);
      adv11 = ADS3.toVoltage(adc11);
   }
   else{
      adv8 = -127; adv9 = -127; adv10 = -127; adv11 = -127;  
   }//end else

   if(!ads4_err){                            // if adc1115 0x4B is conected
      adc12 = ADS4.readADC(0);               // read A/D to number
      adc13 = ADS4.readADC(1);
      adc14 = ADS4.readADC(2);
      adc15 = ADS4.readADC(3);
      adv12 = ADS4.toVoltage(adc12);         // convert number to voltage
      adv13 = ADS4.toVoltage(adc13);
      adv14 = ADS4.toVoltage(adc14);
      adv15 = ADS4.toVoltage(adc15);
   }
   else{
      adv12 = -127; adv13 = -127; adv14 = -127; adv15 = -127;  
   }//end else
   #ifdef DEBUG
       Serial.println(F("ADV1115 A/D: 0-15 (V)")); Serial.flush();
       Serial.println(adv0); Serial.println(adv1); Serial.println(adv2); Serial.println(adv3);
       Serial.println(adv4); Serial.println(adv5); Serial.println(adv6); Serial.println(adv7);
       Serial.println(adv8); Serial.println(adv9); Serial.println(adv10); Serial.println(adv11);
       Serial.println(adv12); Serial.println(adv13); Serial.println(adv14); Serial.println(adv15);
   #endif            
}//end void
...
bool get_ap_button(){
  ...
  bool button;
  button = digitalRead(input_ap_manager);
  delay(1);
  if(button) apm = true; // ap manager button not pressed
  if(!button && apm){    // ap manager button is pressed and previous not pressed
    apm = false;
    #ifdef DEBUG
        if(hw_board_version==1) Serial.println(F("BUTTON AP MANAGER"));
        else                    Serial.println(F("BUTTON AP MANAGER or set RELAY 1 high/low"));
    #endif

    if(hw_board_version == 0){ // HW board is 1.0
       change = !change;
       set_rele(change);
       if(change){ 
         digitalWrite(out_led, HIGH);
  ...
  button = digitalRead(input_ap_manager);
  if(!button){                                 // confirmed AP manager button
    set_rele(0);                               // relay off
    change = false;
    #ifdef DEBUG
...
void set_rele_2(byte s){  // relay 2
   relay_status_2 = s;
   if(revers && s==0) digitalWrite(out_relay_2, HIGH);      // on if revers
   else if(revers && s==1) digitalWrite(out_relay_2, LOW);  // off if revers
   else if(!revers && s==0) digitalWrite(out_relay_2, LOW); // off
   else if(!revers && s==1) digitalWrite(out_relay_2, HIGH);// on  
}// end void
...
static const char html_ap_saved[] PROGMEM = R"=====(<div></br><b>Settings saved!</br>It is necessary to restart the sensor</b></div></br><form action='/' method='get'><button>Back</button></form>)=====";
...
void WIFIAP_setup(){
     ...
     if(relay_status_2==1){
        content.replace("{j}", "rel?g=0");
        content.replace("{k}", "OFF");
     }
     else{
        content.replace("{j}", "rel?g=1");
        content.replace("{k}", "ON");         
     }       
     ...
     if(server.hasArg("q")){        
        ...
        if(t==1){
           set_rele(1);                                         // relay on
           #ifdef DEBUG
              Serial.println(F("AP RELAY 1: ON"));
              Serial.flush();
           #endif  
        ...
        if(t==0){
           set_rele(0);                                         // relay off
           #ifdef DEBUG
              Serial.println(F("AP RELAY 1: OFF"));
              Serial.flush();
           #endif    
           server.sendHeader("Location", String("/"), true);
     ...
     // relay 2
     if(server.hasArg("g")){        
        int u = atoi(server.arg("g").c_str());    
        if(u==1){
           set_rele_2(1);                                         // relay on
           #ifdef DEBUG
              Serial.println(F("AP RELAY 2: ON"));
              Serial.flush();
           #endif  
           server.sendHeader("Location", String("/"), true);   
           server.send(302, "text/plain", "");                    // redirect to home
        ...
        if(u==0){
           set_rele_2(0);                                         // relay off
           #ifdef DEBUG
              Serial.println(F("AP RELAY 2: OFF"));
              Serial.flush();
           #endif    
           server.sendHeader("Location", String("/"), true);
           server.send(302, "text/plain", "");      
    ...
    content += FPSTR(html_ap_style);
    content += FPSTR(html_ap_header_end);
    content += String(F("<h3>Sensor Information</h3>"));
    content += F("<dl>");
    content += F("<dt><b>Sensor Name</b></dt><dd>");
    content += String(SEN_NAME);
    content += F("</dd>");
    content += F("<dt><b>Sensor FW</b></dt><dd>");
    content += str_fw_version();
    content += F("</dd>"); 
    content += F("<dt><b>Sensor HW</b></dt><dd>");
    if(hw_board_version==1) content += "1.1";
    else content += "1.0";
    content += F("</dd>");       
    ...
    switch(SEN_TYPE){
      ...
      case 7: content += F("Multi contact");  break;
    }
    content += F("</dd>");   
    ...
    if(SEL_MOIST_SONIC==2){
      content += F("<dt><b>Soil ADS1115 probe</b></dt><dd>");
      content += F("Yes</dd>");        
    }
    ...
    if(SEL_MOIST_SONIC==2){
      content += F("<dt><b>Soil probe 1-16 A/D</b></dt>");
      content += F("<dd>");
      content += String(adv0)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv1)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv2)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv3)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv4)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv5)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv6)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv7)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv8)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv9)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv10)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv11)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv12)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv13)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv14)+"V";
      content += F("</dd>");
      content += F("<dd>");
      content += String(adv15)+"V";
      content += F("</dd>");                                
    }          
    ...
    content += F("<option value=\"7\" "); if(SEN_TYPE == 7) content += F("selected");
    content += F(">Multi contact</option>");    
    content += F("</select>");  
    content += F("</dd>");  
    content += F("</dl>");       
    ...
    content += String(DS4_CALIB); 
    content += F("'></dd>");            
    content += F("<dt><b>I2C connector has a function</b></dt><dd>");
    content += F("<select name=\"w\" id=\"w\">");
    content += F("<option value=\"0\" "); if(SEL_MOIST_SONIC == 0) content += F("selected");
    content += F(">Moisture probe</option>");
    content += F("<option value=\"1\" "); if(SEL_MOIST_SONIC == 1) content += F("selected");
    content += F(">Sonic probe</option>");
    content += F("<option value=\"2\" "); if(SEL_MOIST_SONIC == 2) content += F("selected");
    content += F(">Soil probe</option>");           
    content += F("</select>");  
    content += F("</dd>");  
    content += F("</dl>");      
    content += F("<dt><b>Moisture probe</b></dt><dd>");
    ...
    if(server.hasArg("w")){
      setEEPROMString(offsetof(eepromdata_t, SEL_MOIST_SONIC), elementSize(eepromdata_t, SEL_MOIST_SONIC), server.arg("w"));
      #ifdef DEBUG
        Serial.print(F("Moist, Sonic, Soil: "));
        Serial.println(server.arg("w"));
        Serial.flush();
      #endif
    }           
