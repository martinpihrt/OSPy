 ...
 * Martin Pihrt - 08.03.2021
 * Arduino IDE >= 1.8.13
 * board ESP32 1.0.5 (https://github.com/espressif/arduino-esp32)
 * 
 ...
 *  Manual switch relay to on/off with AP button (if press time < 2 sec)
*/
...
// FW version
int fw_version = 105;        // ex: FW version 105 is 1.05
/* 
...
// I2C
byte I2C_SDA           = 33;                     // SDA (and not I2C moisture probe DHT22...)
byte I2C_SCL           = 32;                     // SCL
...
typedef struct{
  ...
  char INVERT_DRY[3];     // dry contact inverted logic 0->1
  char INVERT_MOTI[3];    // motion contact inverted logic 0->1
  char MOIST_TYPE[2];     // moisture probe type 0=DHT22, 1=DHT21, 2=DHT11
  char UPLOAD_PASS[32];   // max 32 char password for uploading access in STA mode via http from OSPy
  char AP_WIFI_PASS[32];  // max 32 char password for Wi-Fi in AP mode
} eepromdata_t;
...
void setup() {
 ...
 -> FALLING Triggers interrupt when the pin goes from HIGH to LOW
  ...
  if(MOIST_TYPE==0){ // DHT22 (AM2302), AM2321 probe
     //DHT dht(I2C_SDA, DHT22);
  }
  if(MOIST_TYPE==1){ // DHT 21 (AM2301) probe
     //DHT dht(I2C_SDA, DHT21);
  }
  if(MOIST_TYPE==2){ // DHT 11 probe
     //DHT dht(I2C_SDA, DHT11);
  }
  if(MOIST_TYPE==3){ // SHT21 (HTU21D)
     htu.begin();
  }   
...
float battery_read(){
    ...
    //float output = 0.0;           // output value in %
...
static const char html_ap_saved[] PROGMEM = R"=====(<b>Settings saved!</br>It is necessary to restart the sensor</b></div></br><form action='/' method='get'><button>Back</button></form>)=====";
