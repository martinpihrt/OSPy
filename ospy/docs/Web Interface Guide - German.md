...
[Mobile API v1-Dokumentation](../../api/docs/Mobile_API_v1.md), die auch in
der OSPy-Hilfe unter **API** verfügbar ist.

## Mobile Anwendungen und sofortige Benachrichtigungen

Normale App-Kommunikation und Push-Zustellung sind getrennte Wege. Im normalen Betrieb verbindet sich die Android-App mit der für die Installation gespeicherten lokalen oder entfernten Adresse, prüft HTTPS, authentifiziert sich mit installationsbezogenen Mobile-API-v1-Token, liest den aktuellen Zustand und sendet erlaubte Befehle direkt an OSPy.

Für Push wartet OSPy nicht auf eine Verbindung der App. Ein passendes Ereignis wird in eine begrenzte Hintergrundwarteschlange gestellt; OSPy signiert die HTTPS-Anfrage an den Relay, der Relay prüft sie und veranlasst Firebase Cloud Messaging, Android zu wecken und die Systembenachrichtigung anzuzeigen. Fehler blockieren die Bewässerung nicht.

Unter **Optionen → Mobile Anwendungen**, zwischen **Benutzer** und **Sicherheit**, konfiguriert ein Administrator das HTTPS-Push-Relay und aktiviert sofortige Benachrichtigungen. Push bleibt standardmäßig deaktiviert, während das bearbeitbare Relay-Feld mit der offiziellen OSPy-Relay-Adresse vorbelegt ist. Die Seite verwendet dieselbe Liste gekoppelter Geräte wie die mobile API und zeigt Benutzer, Rolle, App-Version, Kopplungszeit, letzte Aktivität, Push-Status sowie die letzte erfolgreiche oder fehlgeschlagene Zustellung. Kategorien für Stationsstart, Stationsstopp, Regen, Diagnose, Aktualisierungen und sonstige Ereignisse sind pro Telefon wählbar. Außerdem können ein Test gesendet, Push abgemeldet oder das gesamte Gerät einschließlich seiner Anmeldetoken widerrufen werden. Ein aktives Gerät muss vor dem endgültigen Löschen seines Datensatzes zuerst widerrufen werden; widerrufene Datensätze können einzeln oder gemeinsam gelöscht werden. Eine erneute Anmeldung derselben gespeicherten Android-Installation verwendet deren bestehende Identität und ersetzt die Token, ohne eine weitere Zeile anzulegen.
...
Das optionale Plug-in **Automation Rules** erstellt grafische AND/OR-Regeln aus zwischengespeicherten Werten von Überwachungs-Plug-ins, aktivierten Pihrt- oder Shelly-OSPy-Sensoren, lokalem Datum und Uhrzeit sowie schreibgeschützten OSPy-Systemzuständen. Eine Regel kann auf dauerhaft erfüllte Bedingungen warten, Warnungen wiederholen und die Rückkehr zum Normalzustand über Home, Browser, E-Mail, Telegram oder mobilen Push melden. Ein separater, standardmäßig deaktivierter Steuerschalter erlaubt geschützte Aktionen: ausgewählte Stationen, alle Ausgänge oder laufende Programme stoppen, den Planer deaktivieren, einen bestimmten Ausgang im manuellen Modus einmalig oder in einem zeitlich begrenzten Lauf/Pause-Zyklus einschalten, die globale Wasserstufe ändern oder eine von einem anderen Anbieter ausdrücklich deklarierte Aktion aufrufen. Der Aktionsverlauf protokolliert jeden Zyklusstart, jede Pause, jeden erneuten Lauf, Fehler und den endgültigen Stopp samt Grund. Water Meter 1.2.1 stellt die erste konkrete Anbieteraktion zum ausdrücklichen Zurücksetzen seiner Verbrauchszähler bereit. Der Testmodus simuliert Aktionen nur und protokolliert die Ergebnisse. Eine optionale Vorfallsperre erfordert die Bestätigung durch einen Administrator, nachdem alle Bedingungen verfügbar und inaktiv sind; das Entsperren macht ausgeführte Aktionen niemals rückgängig.
...
Beim Start vergleicht OSPy die maßgeblichen shelve-Werte mit den im Schattenschema 2 gespeicherten SHA-256-Prüfsummen. Bei diesem automatischen Vergleich werden keine Pickle-Werte aus der Schattenkopie deserialisiert und niemals zur Wiederherstellung oder zum Start von OSPy verwendet. Eine übereinstimmende Kopie wird als geprüft markiert; fehlende, geänderte oder unerwartete Schlüssel sowie Schäden an Prüfsummen oder Datenbank werden als Warnung gemeldet, während shelve weiter verwendet wird. Eine Schattenkopie mit Schema 1 aus der vorherigen Übergangsphase wird nicht gelesen und beim nächsten Speichern sicher ersetzt.

Beim Start und nach jedem erfolgreichen Ersetzen der Schattenkopie führt OSPy eine probeweise Dekodierung erst dann durch, wenn alle Schlüssel und Prüfsummen in einem einzigen schreibgeschützten Abbild übereinstimmen, und wendet anschließend die normale Einstellungsvalidierung auf die rekonstruierten SQLite-Werte an. Das rekonstruierte Wörterbuch wird verworfen und niemals der laufenden Konfiguration zugewiesen. Die Diagnose meldet, ob dieser SQLite-Lesetest bestanden wurde; ein Fehler bleibt eine Warnung und OSPy arbeitet ausschließlich mit den bereits aktiven shelve-Einstellungen weiter.

Schattenschema 3 enthält zusätzlich ein Manifest mit der Gesamtzahl der Datensätze und einer Prüfsumme über die vollständige sortierte Liste aus Schlüsseln und deren Prüfsummen. Dadurch werden entfernte oder hinzugefügte Zeilen vor der Dekodierung unabhängig erkannt. OSPy führt für die aktuelle SQLite-Schattenkopie und die vorherige Kopie im Sicherungsverzeichnis einen vollständigen Wiederherstellungs-Probelauf durch und verwirft dessen Ergebnis. Die Diagnose zeigt beide Ergebnisse. Ein fehlerhafter oder nicht verfügbarer Wiederherstellungskandidat ändert niemals die Startauswahl; shelve/DBM bleibt maßgeblich.

Beim Start verwendet OSPy den ersten unabhängig geprüften SQLite-Kandidaten ausschließlich für eine Wiederherstellungsprobe in einem isolierten temporären Verzeichnis. Daraus wird eine temporäre shelve/DBM-Datenbank erstellt, erneut geöffnet, vollständig validiert und anhand aller serialisierten Einstellungswerte verglichen; anschließend wird das Verzeichnis entfernt. Weder die aktive Datenbank noch die Einstellungsquelle beim Start werden geändert. Ist die aktuelle SQLite-Kopie beschädigt, kann die Probe die geprüfte Sicherung verwenden, während die laufenden Einstellungen weiterhin aus dem maßgeblichen shelve geladen werden. Die Zeile **Datenbank** in der Diagnose zeigt Ergebnis, Quelle und Anzahl der Einstellungen.

OSPy simuliert außerdem lediglich die Notfallentscheidung für den Fall, dass alle shelve/DBM-Datenbanken ungültig wären. Die unabhängig geprüfte aktuelle SQLite-Kopie hat Vorrang; die geprüfte Sicherung würde nur bei einem Fehler der aktuellen Kopie ausgewählt. Das ausgewählte Wörterbuch wird verworfen und niemals den laufenden Einstellungen zugewiesen, daher wird auch in diesem Zustand noch keine SQLite-Wiederherstellung aktiviert. Die Diagnose zeigt Bereitschaft, hypothetische Quelle und Anzahl der Einstellungen.

Die experimentelle Option **Notfallwiederherstellung der SQLite-Einstellungen zulassen** unter **Einstellungsspeicher** ist standardmäßig deaktiviert. Nach dem Aktivieren erstellt OSPy einen nicht geheimen atomaren Aktivierungsmarker erst, wenn eine SQLite-Kopie mit Schema 3 unabhängig geprüft wurde; auch die geprüfte Kopie selbst muss die aktivierte Option enthalten, sodass weder Marker noch Datenbank allein eine Wiederherstellung erlauben. Sie wird nur versucht, wenn aktuelle, temporäre und gesicherte shelve/DBM-Datenbank ungültig sind. OSPy bevorzugt die aktuelle SQLite-Kopie, verwendet ersatzweise die geprüfte Sicherung, baut im temporären Verzeichnis eine shelve-Datenbank neu auf, prüft sie erneut und lädt erst danach. Bestehende aktuelle und Sicherungsverzeichnisse werden dabei nicht überschrieben; die normale Speicherung bleibt shelve/DBM. Beim Deaktivieren wird der Marker vor dem Speichern entfernt. Diagnose und Startmeldung zeigen Verwendung oder Fehler; prüfen Sie danach das System und erstellen Sie eine Sicherung.

Die experimentelle Option **Geprüftes Lesen der SQLite-Einstellungen bevorzugen** unter **Einstellungsspeicher** ist ebenfalls standardmäßig deaktiviert. Nach dem Aktivieren lädt und prüft OSPy immer zuerst shelve/DBM, vergleicht die vollständige Schlüssel- und Prüfsummenmenge von SQLite mit genau diesem maßgeblichen Abbild und dekodiert sowie validiert jeden Wert unabhängig. Nur bei vollständiger Übereinstimmung wird das gleichwertige SQLite-Wörterbuch den laufenden Einstellungen zugewiesen. Eine fehlende Kopie, Abweichung, Beschädigung oder Dekodierungsfehler lässt die bereits geladenen shelve-Werte ohne Unterbrechung aktiv. Schreiben, Verzeichnisrotation und Sicherungen verwenden weiterhin ausschließlich shelve/DBM; SQLite bleibt eine geprüfte Schattenkopie. Die Diagnose zeigt Verwendung, Deaktivierung oder Rückfall auf shelve.

Die experimentelle Option **Geprüftes SQLite für Einstellungs-Commits verlangen** unter **Einstellungsspeicher** ist standardmäßig deaktiviert. Nach dem Aktivieren darf ein neues Einstellungsverzeichnis die aktiven Daten erst ersetzen, wenn sowohl die temporäre shelve/DBM-Datenbank als auch ihre SQLite-Kopie erfolgreich geschrieben und unabhängig geprüft wurden. Ist SQLite nicht verfügbar oder schlagen Schreiben beziehungsweise Prüfung fehl, entfernt OSPy das gesamte nicht bestätigte temporäre Verzeichnis, erhält die bisherigen aktiven Einstellungen und den letzten Speicherzeitpunkt und meldet einen Schreibfehler. Das Deaktivieren stellt das kompatible shelve-zuerst-Verhalten wieder her. Ein erfolgreicher strenger Commit kann über den geprüften SQLite-Pfad gelesen werden, shelve/DBM bleibt jedoch Speicher- und Sicherungsformat. Die Diagnose zeigt die aktive Commit-Richtlinie.
...
Der Selektor **Einstellungsspeichermodus** fasst die sicheren Übergangsoptionen zusammen. **Kompatibel** behält shelve/DBM als maßgeblichen Speicher und SQLite als optionale Schattenkopie. **Prüfung** aktiviert Notfallwiederherstellung, geprüfte Lesevorgänge und striktes Doppelschreiben; auch dabei wird SQLite nicht zur primären Datenbank. Die einzelnen Schalter bleiben für die erweiterte Steuerung verfügbar, eine gemischte Kombination wird automatisch als **Benutzerdefinierte erweiterte Einstellungen** bezeichnet. Bestehende Installationen leiten den Modus aus den gespeicherten Schaltern ab, ohne deren Werte zu ändern.

Die Diagnose bewertet zusätzlich eine passive **SQLite-Primary-Beta-Bereitschaft**. Bereit erfordert den Prüfmodus, eine verifizierte aktuelle Schattenkopie, erfolgreiche Lese- und Wiederherstellungstests der aktuellen und gesicherten Kopie, eine erfolgreiche Wiederherstellungsprobe und Notfallauswahl, aktivierte Notfallwiederherstellung, verifiziertes Lesen und striktes Doppelschreiben sowie mindestens fünf aufeinanderfolgende verifizierte Starts und zwanzig strikte Schreibvorgänge. Bis dahin zeigt die Diagnose die noch laufende Beweissammlung oder die blockierenden Prüfungen. Diese Kontrolle wechselt das Speicher-Backend niemals automatisch; shelve/DBM bleibt maßgeblich.
...
Die Seite verwendet die aufklappbaren Bereiche **Systemstatus**, **Leistung und Prozesse** und **Sicherheitsprüfung**. Durch Anklicken der dunklen Bereichsleiste wird ein Bereich ein- oder ausgeklappt. **Systemstatus** weist jedem Betriebsbereich den Zustand OK, Warnung, Fehler oder nicht konfiguriert zu. Überwacht werden der Heartbeat des Schedulers, die letzte erfolgreiche Zeitplanberechnung und der nächste Lauf, Ausgabebefehle, Sensorkommunikation, der Zugriff auf die Einstellungsdatenbank, freier Speicherplatz, Fehler aktivierter Plug-ins, E-Mail-Bereitschaft, Wetter, Internetverbindung und die neueste verfügbare Sicherung. Ein erfolgreicher Ausgabeschreibvorgang bestätigt nur, dass OSPy den angeforderten Zustand an den Treiber übergeben hat; ohne Hardware-Rückmeldung wird kein physisches Schalten des Relais bestätigt.

Die letzte Zeile **Betriebssystemzustand** führt im Hintergrund eine fehlertolerante Prüfung von Linux und dem Root-Speicher aus und speichert das Ergebnis zehn Minuten lang zwischen. Die Gesamtzeile verwendet Grün/OK, Gelb/Warnung und Rot/Kritisch; jede Einzelprüfung ist zusätzlich als OK, Warnung, Kritisch oder Information gekennzeichnet. Geprüft werden Einbindungsmodus des Root-Dateisystems, Speicherplatz, Inodes, ein kleiner synchronisierter Schreib-/Lesetest, verfügbare EXT4-Metadaten und SD-Kartenidentität, Unterspannung und Throttling des Raspberry Pi, CPU-Temperatur, Speicher, Last, fehlgeschlagene systemd-Dienste sowie ein begrenzter Kernel-Log-Ausschnitt auf MMC-, I/O-, EXT4-, Nur-Lese-Remount-, Panic-, Lockup- und Speichermangelfehler. Fehlende Befehle oder Berechtigungen werden angezeigt, ohne OSPy anzuhalten. Die Prüfung verwendet kein sudo und führt niemals fsck oder eine andere Reparatur am eingebundenen Root-Dateisystem aus.

Der Bereich **So funktioniert OSPy** bietet interaktive Diagramme für Programme und Planung, Startentscheidungen von Stationen, Master-Stationen, Wetter, Sensoren, den Plug-in-Lebenszyklus, sichere Aktualisierungen, die Speicherung von Einstellungen sowie Sicherung und Wiederherstellung. Diagramme können gezoomt, in voller Breite geöffnet, gedruckt oder als SVG bzw. PNG heruntergeladen werden. Knoten mit gestricheltem Rand öffnen die zugehörige OSPy-Seite; eine separate Schaltfläche führt zur passenden Hilfe. Der Mermaid-Renderer ist lokal in OSPy gespeichert und benötigt keinen Internetzugang.

Zusätzliche Sicherheitsthemen erklären den vollständigen geschützten Web-Anfragepfad, HTTPS und Zertifikate, die Anmeldung mit Zwei-Faktor-Authentifizierung, Sitzungen mit CSRF- und Remember-me-Token sowie die Sicherheit von API und Sensoren. Die Diagramme unterscheiden Authentifizierung, Rollenberechtigung und CORS, zeigen den Brute-Force-Schutz und weisen darauf hin, dass OSPy nur einen Hash des geheimen Teils eines langfristigen Anmeldetokens speichert.

Weitere Diagramme behandeln Priorität und Parallelität der Bewässerung, Regen und Wasserbilanz, den Befehlsweg zur Ausgangshardware, Benutzerrollen, Ereignisse und Vorfälle, die Zustellung von Benachrichtigungen, Start und Beenden von OSPy, die Genehmigung von Plug-in-Berechtigungen, den vollständigen Sensorlebenszyklus und den sicheren Netzwerkzugriff. Das Diagramm **Saubere OSPy-Installation** folgt dem tatsächlichen abgesicherten Installationsprogramm von den Linux- und Python-Prüfungen über den stabilen `master`-Zweig, Abhängigkeiten, SQLite und systemd bis zur ersten Anmeldung und Änderung des erzeugten Passworts.

Die Zeile **Sprachen** vergleicht jeden Übersetzungskatalog mit der aktuellen Vorlage und zeigt für jede Sprache den übersetzten Prozentsatz sowie die genaue Anzahl fehlender Zeichenketten. Eine vollständige Übersetzung ist grün, eine Abdeckung ab 80 % gelb und eine geringere Abdeckung rot. Dies dient nur zur Information der Übersetzungsverantwortlichen: Der Sprachstatus ändert weder den Gesamtzustand des Systems noch öffnet er das Diagnosefehlerfenster.

Die **Sicherheitsprüfung** bewertet passiv HTTPS, anonymen Zugriff, Zwei-Faktor-Authentifizierung, die Authentifizierung der Sensor-API, den CSRF-Schutz der API, CORS, das Sensorpasswort und empfohlene HTTP-Sicherheitsheader. Sie zeigt das Sensorpasswort niemals an und ändert keine Einstellung. Das Profil **Heimnetzwerk** bewertet ausgewählte Kompromisse zur Kompatibilität als Warnung, während **Internetzugriff** verschlüsselten, authentifizierten und eingeschränkten Zugriff verlangt. Jeder Befund enthält den aktuellen Zustand, eine Empfehlung und einen Link zur zugehörigen Einstellung. Ein Reverse-Proxy kann Sicherheitsheader ergänzen, nachdem die Antwort OSPy verlassen hat; die Diagnose meldet nur, was OSPy selbst garantieren kann.

Abgefangene Betriebsfehler des Kerns erscheinen im **Systemstatus** als eigene aktive Probleme. Jeder Eintrag nennt die betroffene Komponente, die Anzahl der Vorkommnisse, das letzte Auftreten, die technische Ursache und eine empfohlene Behebung. Ein erfolgreicher erneuter Versuch entfernt das aktive Problem automatisch; der vollständige Traceback bleibt im Ereignisprotokoll erhalten. Erwartete Kompatibilitäts-Fallbacks werden nicht als Fehler gemeldet.

Der aufklappbare Bereich **Vorfallverlauf** bewahrt bis zu 200 aktuelle Probleme über OSPy-Neustarts hinweg auf. Gefiltert werden können alle, aktive, behobene und durch einen Neustart unterbrochene Vorfälle. Eine Wiederholung desselben aktiven Problems erhöht den Zähler, statt Duplikate anzulegen; die Tabelle zeigt Öffnung, letztes Auftreten und Behebung. Der Verlauf wird atomar in einer validierten, größenbegrenzten JSON-Datei gespeichert und dupliziert keine vollständigen Tracebacks.
...
Das Manifest kann in der Liste `dependencies` Abhängigkeiten von anderen Plug-ins anhand ihres Verzeichnisnamens deklarieren. Eine erforderliche Abhängigkeit muss installiert und aktiviert sein. Eine optionale Abhängigkeit blockiert das Plug-in nicht; wenn jedoch beide Plug-ins aktiviert sind, startet OSPy zuerst den Anbieter. Beim Beenden gilt die umgekehrte Reihenfolge, und Abhängigkeitszyklen werden nicht gestartet. Bei einer Masseninstallation kann eine erforderliche Abhängigkeit im selben ZIP enthalten sein; bei alleiniger Installation des abhängigen Plug-ins muss der Anbieter bereits installiert sein.
...
Ab OSPy 3.0.294 muss ein Administrator die im Manifest angegebenen Berechtigungen eines neu installierten Plug-ins ausdrücklich genehmigen, bevor dessen Code gestartet werden kann. Die einmalige Upgrade-Migration übernimmt automatisch die Genehmigungen aller bereits installierten Plug-ins, einschließlich deaktivierter Plug-ins, sodass ein OSPy-Update bestehende Installationen nicht unterbricht. Ein unveränderter oder kleinerer Berechtigungsumfang bleibt genehmigt; eine zusätzliche Berechtigung blockiert manuelle und automatische Aktualisierungen, bis der Administrator den neuen Umfang geprüft und genehmigt hat. Das Widerrufen der Genehmigung deaktiviert das Plug-in. Die Genehmigung wird in OSPy als Administratorentscheidung und Prüfprotokoll gespeichert; sie erzeugt keine Betriebssystem-Sandbox.
...
Wenn ein I2C-Plug-in eine von mehreren alternativen Adressen verwenden kann, behandelt die Kompatibilitätsprüfung die Deklaration als eine auswählbare Ressource und nicht als gleichzeitige Belegung aller Adressen. Die Installation aus dem Repository oder einem eigenen ZIP ist zulässig, wenn eine eindeutige Adresse verfügbar bleibt. Bei der Aktivierung behält das Plug-in seine bevorzugte Adresse oder wählt eine freie Alternative. Seine Einstellungen weisen eine Adresse zurück, die bereits von einem anderen aktivierten Plug-in verwendet wird, behalten alle vorherigen Einstellungen bei und zeigen den Konflikt in einer roten Statusleiste an, ohne die Seite zu verlassen.

### Benutzerdefiniertes Plug-in (ZIP)

OSPy prüft das vollständige ZIP, bevor eine Plug-in-Datei geschrieben wird. Das Archiv muss mindestens ein Plug-in-Verzeichnis mit `__init__.py` und einer gültigen UTF-8-Datei `plugin.json` enthalten, deren `id` dem Verzeichnisnamen entspricht. Ein Plug-in darf außerdem Python-Module, `README.md` sowie die Verzeichnisse `templates`, `static`, `script`, `docs`, `i18n` und `data` enthalten. Ein Repository-Archiv darf Plug-ins unter `plugins/` ablegen; ein eigenes Archiv darf ein Plug-in direkt im Stammverzeichnis enthalten.

Absolute Pfade, Pfade in übergeordnete Verzeichnisse, nicht portable Namen, doppelte Pfade oder Plug-in-Kennungen, symbolische Links, spezielle oder verschlüsselte Einträge und beschädigte Archive werden abgelehnt. Die Grenzwerte betragen 64 MiB heruntergeladene ZIP-Daten, 4.096 Archiveinträge, 256 Plug-ins, 32 MiB je entpackter Datei, insgesamt 128 MiB entpackte Daten und ein Kompressionsverhältnis von höchstens 200:1. Die Installation erfolgt je Plug-in atomar: Dateien werden zuerst vorbereitet, das vorhandene Verzeichnis `data` bleibt erhalten und bei einem Fehler beim Austausch oder Start wird die vorherige Version wiederhergestellt.
...
Ein Plug-in kann mit `showOnTimeline` einen Live-Wert einer bestimmten Station zuordnen. OSPy aktualisiert ihn alle drei Sekunden neben dem Status der laufenden Station, einschließlich Hauptstationen und manuellem Modus. Der Wert verschwindet, wenn die Station stoppt oder das Plug-in ihn löscht; die Anzeige verändert weder die Ausgangssteuerung noch die Zeitplanung.
...
### Zusätzliche Benutzer
Die responsive Benutzerverwaltung zeigt den integrierten Hauptadministrator und alle zusätzlichen Konten mit Rolle, Notiz und 2FA-Status. Administratoren können zusätzliche Konten erstellen, bearbeiten, löschen oder deren 2FA zurücksetzen. Beim Bearbeiten bleibt das vorhandene Passwort erhalten, wenn das Feld für das neue Passwort leer bleibt.

Der integrierte Hauptadministrator verwendet weiterhin die bisherigen globalen OSPy-2FA-Einstellungen. Jedes zusätzliche Konto kann TOTP oder E-Mail-Verifizierung unabhängig über **Kontosicherheit** im Seitenkopf konfigurieren. Ältere Konten bleiben gültig und beginnen mit deaktivierter 2FA. Ein Administrator kann eine verlorene 2FA-Kopplung zurücksetzen, sieht jedoch weder das Geheimnis noch die Sicherungscodes des Benutzers.
...
Sicherungsformat 2 speichert das maßgebliche Einstellungs-Backend, den Speichermodus und das SQLite-Schema. Vor dem Archivieren der laufenden Einstellungen speichert OSPy ausstehende Änderungen und blockiert gleichzeitige Einstellungs-Schreibvorgänge. Die SQLite-Datenbank wird transaktional über die SQLite Backup API kopiert und das Abbild unabhängig auf Integrität, Schema, Datensatzmanifest und Wertprüfsummen geprüft. Die Wiederherstellung wiederholt diese logische Prüfung nach dem Entpacken und weist ein Abbild zurück, dessen deklarierter Modus, Schema oder Einstellungsanzahl nicht übereinstimmt. Manifest-Sicherungen im Format 1 und ältere Sicherungen ohne Manifest bleiben wiederherstellbar.
Aktive Websitzungen werden weder gesichert noch wiederhergestellt; nach der Wiederherstellung müssen Sie sich erneut anmelden.
