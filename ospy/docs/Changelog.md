...
Added the plug-in repository changelog to the Help page Plug-ins section when `plugins/CHANGELOG.md` is available, and copy the plug-in repository README and changelog into the local `plugins` directory during plug-in repository checks, installs, or updates.
