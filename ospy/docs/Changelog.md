...
Expanded Diagnostics → **How OSPy works**. The login diagram now distinguishes the legacy built-in administrator settings from independent additional-user 2FA. Two new interactive diagrams explain direct Android communication with Mobile API v1 and the separate asynchronous push path from an OSPy event through the signed HTTPS relay, Firebase Cloud Messaging and Android background delivery. Updated the root documentation and all seven Web Interface Guides with both mobile paths.
...
Added one-time refresh-token recovery for the short interruption window between server-side rotation and durable Android storage. A Google Play update or operating-system process termination in that window no longer leaves a visible saved installation with an unusable session; a replayed old token is accepted only once within five minutes, while logout, device revocation, expiry and later replay remain rejected.
...
Made all Mobile API v1 program creation and full updates atomic by validating and building the complete schedule on a detached program before changing the live configuration. Added strict validation for all seven supported scheduling types, stable type-specific editor kinds, detailed rejection reasons and regression coverage preventing failed edits from renaming or converting an existing program. Expanded the Mobile API reference with the exact `type_data` shape for every program type. The Android client now selects a type before creating a program, preserves the type of existing programs, presents a matching native form for each scheduling model, reports a localized reason when saving fails, and increases its version to 0.3.14 (21). Completed the schedule-type chapter in all seven Web Interface Guides with practical descriptions of every program type and field, graphical interval editing, repeated-cycle anchoring, custom schedules and weather-based execution priorities.
...
(Martin Pihrt)<br/>
Water Meter v1.1.0 now uses uninterrupted one-second flow measurements and separates its responsive live overview from configuration. It adds current `l/s` and equivalent `l/min`, current-minute/current-hour/total consumption, local JSON and optional SQL logging, selectable history source, seconds-based interval, zero-flow filtering, bounded or unlimited retention, CSV download, a flow graph, an optional Home value and mobile API v1 cards with bounded history. Updated both README files, in-app help and all seven Web Interface Guides.
...
Added the stable, versioned `/api/v1` JSON API for native mobile clients while preserving the existing `/api`. It provides capability discovery, short-lived signed access tokens, rotating and revocable refresh tokens, administrator 2FA, paired-device management and explicit read, control, configuration, plug-in, backup, update and system scopes. Native resources cover Home status, stations and masters, programs, run-once irrigation, sensors and history, weather, logs, diagnostics, notifications, plug-in health and optional JSON-only plug-in contributions, backups, System Update, restart/reboot/poweroff and persistent operation results. Replayable Server-Sent Events have a polling fallback, responses use consistent request IDs and error envelopes, and an OpenAPI description is built in. Added an API reference to Help and all seven Web Interface Guides plus isolated authentication, scope, rotation, notification, operation and plug-in-contract tests.
...
Plug-in Wind Speed Monitor v1.1.0
...
Added backward-compatible administrator approval for plug-in permissions starting with OSPy 3.0.294. On the first upgraded start, every already installed plug-in, including disabled ones, is approved once so promotion to `master` cannot interrupt existing systems. New installations must be explicitly approved before plug-in code is imported; unchanged or reduced permissions remain approved, while newly requested permissions block activation and automatic updates until reviewed. Approval and revocation are synchronously persisted and recorded in the security event log, failed installations restore the previous approval, revocation disables the plug-in, and Diagnostics reports an enabled plug-in whose approval is missing. Plug-in management and ZIP/repository installation now show translated permission meanings and clear approve actions in all three themes. Added regression coverage and updated the plug-in developer guide, automated-test guide and all seven Web Interface Guides.

Kept the language selector inside the original System card after introducing the separate Settings storage card. Options categories are now guaranteed to form one contiguous section each, preventing a second identically named System card. Added regression coverage for duplicate category runs.

Added an explicitly selected **SQLite primary (beta)** settings-storage mode after the readiness gate is fully satisfied. Entering or leaving the mode creates a complete format-2 system backup, persists a verified strict dual-write snapshot and restarts OSPy. Startup accepts SQLite only with its atomic activation marker, schema-3 manifest, complete checksums, valid settings and all safety controls; any failure immediately loads the synchronized shelve/DBM fallback and reports the reason in Diagnostics. The interface keeps the choice disabled before readiness, the early language bootstrap follows the same guarded source selection, and primary-mode backups preserve and validate both SQLite and its marker while all older backup formats remain restorable. Added activation, startup, corruption-fallback, language, backup/restore, rendering and diagnostics regression coverage and updated all seven Web Interface Guides.
