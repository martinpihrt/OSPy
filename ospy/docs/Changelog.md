...
-----------
(martinpihrt)<br/>
Fixed live home page plug-in data refresh so values are rebuilt from current plug-in runtime data instead of being updated by stale list positions. This prevents data from one plug-in being shown under another plug-in after a plug-in hides or clears its home/footer data. Updated the Thermostat plug-in so stopping a selected program also cancels matching Run-Now activity and turns off active stations belonging to that program. The Thermostat status log now clears the old disabled message when the plug-in is enabled again. Improved the login page reset session action so it closes, removes, and reopens the session database while OSPy is running, which can recover from a stuck session store without restarting OSPy. Improved the Programs page with a compact per-group run-order overview based on the scheduler output, added a divider below group action buttons, and replaced remaining checkboxes in the program editor with switch-style controls.

July 02 2026 (v3.0)
-----------
(martinpihrt)<br/>
Czech and Slovak language update. Added a new Thermostat plug-in for temperature-based program control. The plug-in can use decimal temperature limits with 0.1 °C steps, read temperatures from supported OSPy temperature sources including DS sensors, OSPy sensors, and Shelly Cloud, and start or stop selected OSPy programs when the temperature crosses configured low or high thresholds with hysteresis-style behavior. Updated all web interface guide language versions with a short Thermostat plug-in description. Improved OSPy restart from Options so service-based Linux installations try a system service restart first, while keeping the original process restart fallback for manual runs.

July 01 2026 (v3.0)
-----------
(martinpihrt)<br/>
Fixed language loading during startup so OSPy reads the selected UI language from the same options database locations used by the main options loader, including temporary and backup option databases after an interrupted save or power outage. The i18n loader now also uses absolute paths based on the OSPy installation directory, so translations are loaded correctly even when the service starts with a different working directory. Fixed the home page update notification modal so the "Update system" action includes the required CSRF token, matching the System Update plug-in page behavior and avoiding a forbidden response. Fixed notice-page redirects so they return to the home URL instead of a rendered Python page object, and changed Options reboot/shutdown actions to run after the notice page is returned so web and LCD shutdown messages have time to appear. Czech language update.
Fixed manual mode station countdown refresh so a temporary `/status.json` read error does not stop future countdown updates. Added an optional back-to-top button with left/right placement in Options. Improved modal layering so the update notification stays above page widgets, including the CHMI radar widget on the home page.
Fixed station image upload editing so a replacement image can always be selected, added drag-and-drop upload support, and accepted PNG, JPG, JPEG, and GIF images. Replaced Options page checkboxes with switch-style controls while keeping the existing option form fields and save behavior.

June 30 2026 (v3.0)
-----------
(martinpihrt)<br/>
New language support for German, Polish, Serbian, and Russian (MACHINE TRANSLATION). Added web interface guide documents and translated gettext PO catalogs for these languages. Fixed language switching so the selected language is written to disk immediately before OSPy restarts. Enabled central CSRF enforcement for plug-in POST requests after updating plug-ins to send and verify CSRF tokens. Clarified station handling option labels and help text so the pause between stations is distinguished from master station start/stop offsets, including examples for positive and negative offset values. Updated all web interface guide language versions with the same station pause and master offset explanation. Improved the Options page layout in all built-in themes so longer translated labels have more room, wrap inside their own row, and remain readable on narrow screens. Added compatibility for newer Python versions by providing a local fallback for the removed cgi module used by web.py form handling, while keeping Python 3.11 behavior unchanged. Replaced the removed distutils copy_tree helper with shutil.copytree for backup restore handling. Improved anonymous usage statistics reporting so reports are saved to a local queue before upload, survive connection outages, and are retried when internet access returns or during periodic background checks. Hardened the Raspberry Pi installation script with safer shell options, whiptail bootstrapping, quoted install paths, guarded i2c and user-group setup, safer systemd service generation, daemon reload, and basic reinstall checks for an existing OSPy checkout. Hardened backup restore uploads by validating ZIP member paths and extracting only normalized files inside the intended upload directory. Session cookies now use the Secure flag automatically when OSPy starts with a valid HTTPS certificate, while remaining compatible with HTTP-only installations. OSPy now creates an installation-specific session secret in ospy/data/session_secret, with the previous built-in secret kept only as a fallback if the file cannot be loaded or created. The web login now regenerates the session id after successful password verification to reduce session fixation risk while preserving the current session data. API CORS handling is now centralized and configurable through the Security options, including support for a specific allowed origin list or disabling CORS headers, and legacy JSONP responses are disabled by default unless explicitly enabled. The API CORS and JSONP settings help text and all web interface guide language versions were updated to document the new options. Browser auto-login no longer stores the username and password in localStorage; remembered browsers now use revocable long-term random tokens stored as HttpOnly cookies, with only token hashes saved by OSPy and a Security option to revoke all remembered browser logins. Plugin install and update actions now show the exact source repository before confirmation, log the selected source, display a checking status while loading remote changes, and validate ZIP extraction paths so plugin archives cannot write outside the target plugin directory. The local back_door.py recovery script now warns before resetting credentials, requires confirmation unless --yes is used, disables passwordless access, revokes remembered browser logins, and uses a newly generated recovery password instead of the old admin/admin default.
...
-----------
(martinpihrt)<br/>
Plugin update checking is now triggered immediately after changing the update switches in the plugin manager, and repository downloads use a timeout. The home page update notification modal was improved for long plugin update lists with scrollable content, compact list rendering, close button, and duplicate-message prevention. The plugin manager now has a manual "Load changes" action for checking GitHub without installing updates, and an update-only "Changes" button that shows GitHub commits for the selected plugin path. The plugin changes view now bypasses stale cache when opened, uses the latest plugin commit date for the available version, formats GitHub dates in a readable form, and displays the latest changes in a bounded scrollable list. Login handling was made more robust by separating password verification from form validation, using an explicit redirect after successful login, and preventing duplicate login form submissions. The reset-session action on the login page was changed from a link to a small, subdued gray button. The Run Once page now shows a separate read-only live countdown for active run-once stations, updated from the status API without requiring a page refresh. Manual mode on the home page now updates the station timer fields directly while a manual run is active, changes them to read-only countdown fields, and switches the station button back to off automatically when the timer expires. Czech language translation template was regenerated with updated core and plugin strings.
