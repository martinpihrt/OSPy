...
Volba **Povolit nouzovou obnovu nastavení ze SQLite** v kategorii **Systém** je experimentální a ve výchozím stavu vypnutá. Po zapnutí vytvoří OSPy nesekretní atomický aktivační marker až po nezávislém ověření SQLite schématu 3; povolení musí současně obsahovat i samotná ověřená SQLite kopie, takže marker ani databáze nestačí samostatně. Obnova se použije pouze tehdy, když jsou neplatné aktuální, dočasná i záložní databáze shelve/DBM. OSPy upřednostní aktuální SQLite kopii, případně ověřenou zálohu, obnoví a znovu ověří shelve databázi v dočasném adresáři a teprve tu načte. Stávající aktuální a záložní adresáře tento krok nepřepíše a běžné ukládání zůstává shelve/DBM. Vypnutí odstraní marker ještě před zápisem nastavení. Skutečné použití nebo chyba se zobrazí v Diagnostice a ve zprávě o obnově při startu; po obnově zkontrolujte systém a vytvořte zálohu.

Experimentální volba **Upřednostnit ověřené čtení nastavení ze SQLite** v kategorii **Systém** je také ve výchozím stavu vypnutá. Po zapnutí OSPy vždy nejprve načte a ověří shelve/DBM, potom porovná úplnou sadu klíčů a kontrolních součtů SQLite s tímto přesným autoritativním snímkem a nezávisle dekóduje a ověří každou hodnotu. Teprve při úplné shodě přiřadí ekvivalentní slovník SQLite do běžících nastavení. Chybějící kopie, rozdíl, poškození nebo chyba dekódování ponechá již načtené shelve bez přerušení. Zápisy, rotace adresářů a zálohy nadále používají výhradně shelve/DBM; SQLite zůstává ověřenou stínovou kopií. Diagnostika ukazuje použití, vypnutí nebo návrat k shelve.

Experimentální volba **Vyžadovat ověřené SQLite pro potvrzení nastavení** v kategorii **Systém** je ve výchozím stavu vypnutá. Po zapnutí smí nový adresář nastavení nahradit aktivní data pouze tehdy, když se v dočasném adresáři úspěšně zapíše a nezávisle ověří shelve/DBM i jeho SQLite kopie. Pokud SQLite není dostupné nebo zápis či ověření selže, OSPy odstraní celý nepotvrzený dočasný adresář, zachová předchozí aktivní nastavení i čas posledního uložení a ohlásí chybu zápisu. Vypnutí obnoví kompatibilní chování s prioritou shelve. Úspěšný přísný zápis lze číst ověřenou SQLite cestou, ale shelve/DBM zůstává formátem ukládání a záloh. Aktivní politiku potvrzení zobrazuje Diagnostika.
