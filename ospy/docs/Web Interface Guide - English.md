...
* Water Meter (flow measurement using a water meter with pulse output using the I2C bus)
  Water Meter v1.1.0 measures continuously in one-second intervals and refreshes its separate overview every second. The overview shows current l/s, equivalent l/min, current-minute/current-hour/total consumption and a flow graph. Settings control local JSON and optional SQL logging, the displayed source, interval in seconds, zero-flow filtering, and a maximum record count where 0 is unlimited. The optional Home value displays `current l/s (current l/min)`, and mobile API v1 supplies live cards and bounded history from the selected source.
