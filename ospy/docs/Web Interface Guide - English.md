...
See the complete [Mobile API v1 reference](../../api/docs/Mobile_API_v1.md),
also available in OSPy Help under **API**.
...
The optional **Automation Rules** plug-in creates graphical AND/OR rules from cached monitoring plug-in values, enabled Pihrt or Shelly OSPy sensors, local date/time and read-only OSPy operating states. A rule can wait until conditions remain true, repeat an alert and report recovery through Home, browser, e-mail, Telegram or mobile push. A separate default-off control switch enables guarded actions that stop selected stations, all outputs or running programs, disable the scheduler, operate a specific output once or in a bounded run/pause cycle in manual mode, change the global water-level adjustment or call an action explicitly declared by another provider. Action history records every cycle start, pause, resumed run, failure and final stop with its reason. Water Meter 1.2.1 provides the first concrete provider action for explicitly resetting its accumulated counters. Test mode only simulates actions and records results. Optional incident locking requires administrator acknowledgement after all conditions become available and inactive; unlocking never reverses an executed action.

# Irrigation Safety plug-in
The optional **Irrigation Safety** plug-in combines actual OSPy station and master-output state with cached Water Meter, Pressure Monitor and tank-monitor values. Each enabled station can have an expected minimum and maximum flow, startup delay and fault-confirmation time. Automatic learning accepts samples only after startup delay and while exactly one station runs, then saves a robust range expanded by a configurable safety margin. Fully configured simultaneous stations use the sum of their ranges. The plug-in can also detect flow without irrigation, unavailable flow, missing pressure and a low or unavailable tank value.

Use **Monitor only** while commissioning. **Active protection** can stop all current outputs, switch off the separate master relay, disable the scheduler and keep an incident locked until an administrator acknowledges it after the fault clears. A bounded temporary bypass suppresses control actions but keeps monitoring and notifications active. Safety actions run before asynchronous e-mail or native mobile push delivery. The page refreshes measurements, active stations, learning progress, incidents and bounded history in the background.

Compatible running providers add their own guarded actions. Venetian Blind can open, stop, close or select tilt 1–4; Water Tank Monitor can stop regulation and reset recorded minimum and maximum values; Current Loop Tanks Monitor can stop regulation for one tank or all tanks; and OSPy Backup can create a plug-in data archive.
...
Diagnostics also evaluates a passive **SQLite primary beta readiness** gate. Ready requires Verification mode, a verified current shadow, successful read and recovery checks for both current and backup copies, a successful restore rehearsal and emergency-selection dry run, enabled emergency recovery, verified reads and strict dual-write, plus at least five consecutive verified starts and twenty strict writes. Until then Diagnostics shows whether evidence is still being collected or lists the blocking checks. This gate never switches the storage backend automatically; shelve/DBM remains authoritative.
...
The last **Operating system health** row runs a failure-contained Linux and root-storage check in the background and caches it for ten minutes. Its overall row uses green OK, yellow Warning and red Critical states, while every detailed check is marked OK, Warning, Critical or Information. It checks the root mount mode, disk space, inodes, a small synchronized write/read probe, accessible EXT4 and SD identity data, Raspberry Pi power throttling, CPU temperature, memory, load, failed systemd services and bounded kernel-log evidence of MMC, I/O, EXT4, read-only remount, panic, lockup and out-of-memory failures. Missing commands or permissions are shown without stopping OSPy. The check never uses sudo and never runs fsck or another repair command on the mounted root filesystem.

The **How OSPy works** section provides interactive diagrams for programs and scheduling, station-start decisions, master stations, weather, sensors, the plug-in lifecycle, safe updates, settings storage, and backup and restore. A diagram can be zoomed, opened at full width, printed, or downloaded as SVG or PNG. Nodes with a dotted outline open the related OSPy page, and a separate button opens the relevant help. The Mermaid renderer is stored in OSPy and does not require Internet access.

Additional security topics explain the complete protected web-request path, HTTPS and certificates, login with two-factor authentication, sessions with CSRF and remember-me tokens, and API and sensor security. The diagrams distinguish authentication from role authorization and CORS, show brute-force protection, and note that OSPy stores only a hash of the secret part of a long-term login token.

Further diagrams cover irrigation priority and concurrency, rain and water balance, the output-hardware command path, user roles, events and incidents, notification delivery, OSPy startup and shutdown, plug-in permission approval, the complete sensor lifecycle, and safe network exposure. The **Clean OSPy installation** diagram follows the real guarded installer from Linux and Python checks through the stable `master` branch, dependencies, SQLite and systemd to the first login and replacement of the generated password.
...
Starting with OSPy 3.0.294, an administrator must explicitly approve the permissions declared by a newly installed plug-in before its code can start. The one-time upgrade migration automatically preserves approvals for every plug-in that was already installed, including disabled plug-ins, so updating OSPy does not interrupt existing installations. An unchanged or reduced permission set remains approved; adding a permission blocks manual and automatic updates until the administrator reviews and approves the new set. Revoking approval disables the plug-in. Approval is stored in OSPy as an administrative decision and audit record; it does not create an operating-system sandbox.
...
Backup format 2 records the authoritative settings backend, storage mode and SQLite schema. Before archiving live settings, OSPy saves pending changes and blocks concurrent settings writes. The SQLite database is copied through the transactional SQLite Backup API and the snapshot is independently checked for integrity, schema, record manifest and value checksums. Restore repeats this logical validation after extraction and rejects a snapshot whose declared mode, schema or setting count does not match. Format 1 manifest backups and older backups without a manifest remain restorable.
