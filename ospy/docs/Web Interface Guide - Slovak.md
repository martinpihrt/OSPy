...
Bežná komunikácia aplikácie a doručovanie push sú dve oddelené cesty. Pri bežnej prevádzke sa Android aplikácia spojí s miestnou alebo vzdialenou adresou uloženou pri inštalácii, overí HTTPS, prihlási sa inštalačnými tokenmi Mobile API v1, načíta aktuálny stav a posiela povolené príkazy priamo do OSPy.

Pri push doručení OSPy nečaká na pripojenie aplikácie. Zodpovedajúcu udalosť vloží do obmedzeného frontu na pozadí, podpíše HTTPS požiadavku pre relay, relay ju overí a požiada Firebase Cloud Messaging o prebudenie Androidu a zobrazenie systémovej notifikácie. Chyba relay alebo FCM nezablokuje zavlažovanie.

V **Nastavenia → Mobilné aplikácie**, medzi kartami **Používatelia** a **Zabezpečenie**, správca nastaví HTTPS adresu push relay a zapne okamžité notifikácie. Push zostáva predvolene vypnutý, ale upraviteľné pole relay je vopred vyplnené oficiálnou adresou OSPy relay. Stránka používa rovnaký zoznam spárovaných zariadení ako mobilné API a pri každom telefóne zobrazuje používateľa, rolu, verziu aplikácie, čas spárovania a poslednej aktivity, stav push a posledné úspešné alebo neúspešné doručenie. Možno vybrať kategórie spustenia a zastavenia stanice, dažďa, diagnostiky, aktualizácií a ostatných udalostí, odoslať test, odregistrovať push alebo zrušiť celé zariadenie vrátane prihlasovacích tokenov. Aktívne zariadenie sa musí pred trvalým odstránením záznamu najprv odvolať; odvolané záznamy možno odstrániť jednotlivo alebo všetky naraz. Opätovné prihlásenie z rovnakej uloženej inštalácie Androidu použije jej pôvodnú identitu a nahradí tokeny bez pridania ďalšieho riadka.
...
Spoločný footer OSPy obsahuje priamy odkaz na oficiálnu aplikáciu pre Android v Google Play.
...
Voliteľný doplnok **Automation Rules** vytvára grafické pravidlá AND/OR z hodnôt monitorovacích doplnkov, povolených snímačov OSPy Pihrt alebo Shelly, miestneho dátumu a času a systémových stavov OSPy určených iba na čítanie. Pravidlo môže čakať na trvalé splnenie podmienok, opakovať upozornenie a oznámiť návrat do normálu cez Home, prehliadač, e-mail, Telegram alebo mobilný push. Samostatný, predvolene vypnutý prepínač riadenia povoľuje chránené akcie: zastavenie vybraných staníc, všetkých výstupov alebo bežiacich programov, vypnutie plánovača, jednorazové alebo časovo obmedzené cyklické zapínanie konkrétneho výstupu v ručnom režime, zmenu globálnej úrovne vody alebo vyvolanie akcie výslovne deklarovanej iným poskytovateľom. História akcií ukladá každý začiatok cyklu, pauzu, ďalší beh, chybu a konečné zastavenie vrátane jeho dôvodu. Water Meter 1.2.1 poskytuje prvú konkrétnu akciu poskytovateľa na výslovné vynulovanie nahromadených počítadiel. Testovací režim akcie iba simuluje a ukladá výsledky. Voliteľné uzamknutie incidentu vyžaduje potvrdenie správcu až po sprístupnení a deaktivovaní všetkých podmienok; odomknutie nikdy nevráti vykonanú akciu späť.
...
Posledný riadok **Stav operačného systému** spúšťa na pozadí bezpečne ošetrenú kontrolu Linuxu a koreňového úložiska a výsledok ukladá na desať minút do vyrovnávacej pamäte. Celý riadok používa zelený stav V poriadku, žlté Varovanie a červené Kritické; každá čiastková kontrola je navyše označená ako V poriadku, Varovanie, Kritické alebo Informácia. Kontroluje režim pripojenia koreňového súborového systému, miesto, inody, malý synchronizovaný test zápisu a čítania, dostupné metadáta EXT4 a identitu SD karty, napájanie a throttling Raspberry Pi, teplotu CPU, pamäť, zaťaženie, zlyhané služby systemd a obmedzenú časť kernelového logu pre chyby MMC, I/O, EXT4, opätovné pripojenie iba na čítanie, panic, lockup a nedostatok pamäte. Chýbajúce príkazy alebo oprávnenia sa zobrazia bez zastavenia OSPy. Kontrola nepoužíva sudo a nikdy nespúšťa fsck ani inú opravu na pripojenom koreňovom súborovom systéme.

Sekcia **Ako OSPy funguje** ponúka interaktívne diagramy programu a plánovača, rozhodovania o spustení stanice, hlavnej stanice, počasia, snímačov, životného cyklu doplnkov, bezpečnej aktualizácie, ukladania nastavení a zálohy s obnovou. Diagram možno priblížiť, otvoriť na celú šírku, vytlačiť alebo stiahnuť ako SVG či PNG. Uzly s prerušovaným obrysom otvárajú súvisiacu stránku OSPy a samostatné tlačidlo vedie do príslušnej pomoci. Mermaid renderer je uložený priamo v OSPy a nepotrebuje Internet.

Ďalšie bezpečnostné témy vysvetľujú celú chránenú cestu webovej požiadavky, HTTPS a certifikáty, prihlásenie s dvojfaktorovým overením, relácie s CSRF a zapamätávacími tokenmi a zabezpečenie API aj snímačov. Diagramy rozlišujú autentifikáciu od oprávnenia roly a CORS, ukazujú ochranu proti hrubej sile a uvádzajú, že OSPy ukladá iba hash tajnej časti dlhodobého prihlasovacieho tokenu.

Ďalšie diagramy pokrývajú priority a súbeh zavlažovania, dážď a vodnú bilanciu, cestu príkazu do výstupného hardvéru, roly používateľov, udalosti a incidenty, doručovanie oznámení, spustenie a ukončenie OSPy, schvaľovanie oprávnení doplnkov, celý životný cyklus snímača a bezpečné sprístupnenie v sieti. Diagram **Čistá inštalácia OSPy** opisuje skutočný bezpečný inštalátor od kontroly Linuxu a Pythonu cez stabilnú vetvu `master`, závislosti, SQLite a systemd až po prvé prihlásenie a zmenu vygenerovaného hesla.
...
## Kalendárne pravidlá
Každý existujúci opakovaný program možno ďalej obmedziť kalendárnymi pravidlami. **Povolené mesiace** možno označiť jednotlivo alebo skratkou pre jar, leto, jeseň, zimu a celý rok. **Parita kalendárneho dňa** povolí všetky, nepárne alebo párne dátumy a do poľa **Dni v mesiaci** možno zadať hodnoty od 1 do 31. Pravidlá sa kombinujú s bežnými dňami týždňa alebo intervalom opakovania; predvolené hodnoty zachovajú pôvodný plán.

Do poľa **Vylúčené termíny** sa zadáva jedna položka na riadok: presný dátum (`RRRR-MM-DD`), presný rozsah vrátane krajných dní (`RRRR-MM-DD..RRRR-MM-DD`) alebo každoročne opakovaný rozsah (`MM-DD..MM-DD`). Ročný rozsah môže prechádzať cez Nový rok. Zablokovaný beh zostane v pláne viditeľný s dôvodom. Voľba **Nespúšťať počas štátnych sviatkov** používa lokálne nainštalovaný kalendár sviatkov. Krajinu OSPy zistí z polohy počasia; iný dvojpísmenový kód možno zadať v **Nastavenia → Počasie → Vlastná krajina sviatkov**. Ak knižnica alebo krajina nie sú dostupné, bezpečne sa zablokujú iba programy so zapnutým vylúčením sviatkov; OSPy a ostatné programy pokračujú v prevádzke.

V časti **Servisné odstávky** na stránke Programy možno vytvoriť pomenovaný globálny interval začiatku a konca. Prekrývajúce sa automatické behy sa zobrazia ako blokované, ale **Spustiť teraz** zostane dostupné na zámernú servisnú skúšku. Odstávky zostanú uložené aj po reštarte OSPy.

### Východ a západ Slnka
Programy podľa východu a západu Slnka sú dostupné iba vtedy, keď je zapnutý existujúci doplnok **Sunrise and Sunset**, aktívny jeho výpočet Astral a dostupná platná poloha. Kladný alebo záporný posun v minútach spustí program po vypočítanom jave alebo pred ním. Najskorší a najneskorší povolený čas výsledok obmedzia; beh mimo okna možno presunúť na najbližšiu hranicu alebo preskočiť. Naďalej platia trvanie, pauza, opakovania, dni týždňa aj kalendárne pravidlá. Pri nedostupnom doplnku OSPy nehádá pevný čas a uložený program zachová do obnovenia poskytovateľa.
...
Ak môže I2C doplnok používať jednu z viacerých alternatívnych adries, kontrola kompatibility chápe deklaráciu ako jeden voliteľný prostriedok, nie ako súčasné obsadenie všetkých adries. Inštalácia z repozitára aj vlastného ZIPu je povolená, ak zostáva dostupná odlišná adresa. Pri aktivácii si doplnok ponechá uprednostnenú adresu alebo vyberie voľnú alternatívu. Jeho nastavenia odmietnu adresu, ktorú už používa iný povolený doplnok, zachovajú všetky predchádzajúce nastavenia a zobrazia konflikt v červenom stavovom pruhu bez opustenia stránky.
...
### Ďalši užívateľia
Responzívna správa používateľov zobrazuje vstavaného hlavného správcu aj všetky ďalšie účty vrátane roly, poznámky a stavu 2FA. Správca môže ďalší účet vytvoriť, upraviť, odstrániť alebo mu resetovať 2FA. Pri úprave prázdne pole nového hesla zachová pôvodné heslo.

Vstavaný hlavný správca naďalej používa pôvodné globálne nastavenie 2FA OSPy. Každý ďalší účet si môže nezávisle nastaviť TOTP alebo overenie e-mailom cez **Zabezpečenie účtu** v záhlaví stránky. Staršie účty zostávajú platné a po aktualizácii majú 2FA vypnuté. Správca môže stratené spárovanie 2FA resetovať, ale nevidí tajný kľúč ani záložné kódy používateľa.
...
* Water Meter (meranie prietoku pomocou vodomeru s pulzným výstupom pomocou I2C zbernice)
  Water Meter v1.1.1 meria nepretržite v sekundových intervaloch a samostatný prehľad obnovuje každú sekundu. Prehľad zobrazuje aktuálne l/s, zodpovedajúce l/min, spotrebu v aktuálnej minúte a hodine, celkovú spotrebu a graf prietoku. Nastavenia riadia lokálne JSON a voliteľné SQL protokolovanie, zobrazovaný zdroj, interval v sekundách, vynechanie nulového prietoku a maximálny počet záznamov, kde 0 znamená bez obmedzenia. Voliteľná hodnota na Home zobrazuje `aktuálne l/s (aktuálne l/min)` a mobilné API v1 poskytuje živé karty a obmedzenú históriu z vybraného zdroja. Počítadlo sa číta priamo z registrov PCF8583 0x01–0x03; chyba inicializácie alebo merania zatvorí zbernicu, automaticky zopakuje inicializáciu a zobrazí chybu v prehľade.
* Energy Meter
  Energy Meter v1.0.1 sleduje ľubovoľný počet trojfázových elektromerov Shelly Pro 3EM a Shelly 3EM-63T Gen3. Predvolené a odporúčané pripojenie číta zariadenie priamo v miestnej sieti podľa IP adresy alebo DNS názvu bez cloudu a internetu; alternatívne možno použiť údaje z bežiaceho a nastaveného rozšírenia Shelly Cloud Integration. Každý elektromer má rolu prípojka siete, výroba FVE, samostatná spotreba alebo pomocné meranie a smer možno obrátiť. Prehľad oddeľuje odber a dodávku do siete, zobrazuje L1–L3 a súčet, dnešok, včerajšok, mesiac a rok, náklady, príjem za pretoky, históriu, grafy a CSV. Pri súčasnom meraní siete a FVE vypočíta spotrebu domu, vlastnú spotrebu, energetickú sebestačnosť a úsporu; samotnú dodávku do siete nikdy neoznačuje ako výrobu. Atomicky uložené počítadlá nadviažu po reštarte OSPy a bezpečne rozpoznajú reset alebo výmenu elektromera. Ceny a časové tarify sa zapisujú do každého intervalu, história môže byť v lokálnom JSON, SQL cez Database Connector alebo v oboch úložiskách a údaje sú dostupné na Home, v diagnostike a v mobilnom API.
